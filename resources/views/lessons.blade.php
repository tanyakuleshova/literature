            ...
            <div class="row">
                ...
                <div class="col-lg-4" style="margin-bottom: 30px">
                    <div class="package text-center bg-white">
                        <h3 class="{{$colors_name[$i]}}">{{$lesson['data']}}</h3>
                        <h3 class="{{$colors_name[$i]}}">{{$lesson['name']}}</h3>
                        <p>{{$lesson['description']}}</p>
                        @if($lesson['documents'])
                            @foreach(json_decode($lesson['documents']) as $document)
                                <a href="{{ url('lesson_download/' . $lesson->id . '/'.$document) }}" class="{{$colors_name[$i]}}">{{$document}}</a>
                                <br>
